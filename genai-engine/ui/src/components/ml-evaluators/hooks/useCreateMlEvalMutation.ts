...
import type { CreateMLEvalRequest, Eval } from "@/lib/api-client/api-client";
import { encodePathParam } from "@/utils/url";

export const useCreateMlEvalMutation = (taskId: string | undefined, onSuccess?: (evalData: Eval) => void) => {
  const api = useApi();

  return useApiMutation<Eval, { evalName: string; data: CreateMLEvalRequest }>({
