...
export const TryItOutFormWizardDiscoveryStep: React.FC<DiscoveryStepProps> = ({ group, onBack, isSubmitting }) => {
  ...
  return (
    <Stack spacing={2.25}>
      ...
      <group.Subscribe selector={(s: { values: { competitors: string[] } }) => s.values.competitors.includes("other")}>
        {(show: boolean) =>
          show ? (
            <group.Field name="competitorOther">
              {(field: AnyGroup) => (
                <TextField
      ...
      <group.Subscribe selector={(s: { values: { attribution: string } }) => s.values.attribution === "other"}>
        {(show: boolean) =>
          show ? (
            <group.Field name="attributionOther">
              {(field: AnyGroup) => (
                <TextField
