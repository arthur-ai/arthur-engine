...
export const TryItOutFormWizardAboutStep: React.FC<AboutStepProps> = ({ form, group, onBack }) => {
  ...
  return (
    ...
    >
      <Stack spacing={2.25}>
        ...
        <form.Field name="about.maturity">
          {(field: AnyApi) => {
        ...
        <form.Subscribe selector={(s: AnyApi) => s.values.about.brings === "other"}>
          {(show: boolean) =>
            show ? (
              <form.Field name="about.bringsOther">
                {(field: AnyApi) => (
                  <TextField
