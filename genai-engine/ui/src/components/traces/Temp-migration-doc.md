...
TraceLevel, SpanLevel, SessionLevel, UserLevel	useApi, useTask, useMRTPagination, getFilteredTraces/getFilteredSpans/…, queryKeys, filter store. Build columns via create*LevelColumns({ formatDate, onTrack, Chip }) and render shared TracesTable (or *LevelTable) with data, pagination, isLoading, onRowClick.
...
EvaluatorsSelector	useTask, useApi, useEvals, useEvalVersions, mutation for getLlmEval...Get, withFieldGroup. Passes lists and onAdd/onRemove/onSelect* to EvaluatorsSelectorUI.
