...
export const EvalPickerDialog = ({ open, onClose, onSelect }: EvalPickerDialogProps) => {
  ...
  const handleSearch = useCallback(() => {
    if (searchInput.trim()) {
      setFilters([{ name: "name", operator: TextOperators.CONTAINS, value: searchInput.trim() }]);
    } else {
      setFilters([]);
    }
            ...
            <>
              <TableContainer>
                <Table size="small">
                  ...
                  <TableBody>
                    {evals.map((eval_) => (
                      <TableRow key={eval_.id} hover onClick={() => onSelect({ id: eval_.id, name: eval_.name })} sx={{ cursor: "pointer" }}>
                        <TableCell>
