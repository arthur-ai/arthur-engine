...
export const ImportDatasetModal: React.FC<ImportDatasetModalProps> = ({ open, onClose, onImport, currentRowCount, datasetId, taskId }) => {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>{step === 0 ? CSV_IMPORT_MESSAGES.labels.configureTitle : CSV_IMPORT_MESSAGES.labels.previewTitle}</DialogTitle>
      <DialogContent>
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        {step === 0 ? (
          <>
            ...
            <Button onClick={handleNext} variant="contained" disabled={!selectedFile || isDisabled}>
              {isProcessing ? CSV_IMPORT_MESSAGES.info.processing : CSV_IMPORT_MESSAGES.labels.next}
            </Button>
          </>
        ) : (
          <>
            <Button onClick={handleBack} color="inherit" disabled={isProcessing}>
            ...
            <Button
              ...
              variant="contained"
              color="primary"
              disabled={isProcessing || !previewState?.validation.isValid || previewState.data.totalRows === 0}
            >
              {isProcessing ? CSV_IMPORT_MESSAGES.info.importing : CSV_IMPORT_MESSAGES.labels.import}
            </Button>
          </>
        )}
      </DialogActions>
