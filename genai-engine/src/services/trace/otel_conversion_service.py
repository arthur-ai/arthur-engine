...
class OtelConversionService:
    ...
    def convert_otel_span_to_openinference(
        self, otlp_span: dict[str, Any]
    ) -> dict[str, Any]:
        """Convert full OTLP span to OpenInference span"""
        openinference_span = otlp_span.copy()
        openinference_span["attributes"] = self._convert_genai_to_openinference(
            otlp_span["attributes"]
        )
        return openinference_span

    def _convert_genai_to_openinference(
    ...
    def _get_and_mark(
        self, span_attributes: dict[str, Any], key: str, converted_keys: set[str]
    ) -> Any:
    ...
    def _map_provider_and_system(
        ...
        provider = self._get_and_mark(
            span_attributes, GEN_AI_PROVIDER_NAME, converted_keys
        )
        if provider:
    ...
    def _map_models(
        ...
        request_model = self._get_and_mark(
            span_attributes, GEN_AI_REQUEST_MODEL, converted_keys
        )
        response_model = self._get_and_mark(
            span_attributes, GEN_AI_RESPONSE_MODEL, converted_keys
        )
    ...
    def _map_invocation_parameters(
        self, span_attributes: dict[str, Any]
    ) -> tuple[dict[str, Any], set[str]]:
    ...
    def _map_input_value(
        ...
        input_val = self._get_and_mark(
            span_attributes, "input", converted_keys
        ) or self._get_and_mark(span_attributes, "gen_ai.prompt", converted_keys)
    ...
    def _map_output_value(
        ...
        output_val = self._get_and_mark(
            span_attributes, "output", converted_keys
        ) or self._get_and_mark(span_attributes, "gen_ai.completion", converted_keys)
    ...
    def _map_input_messages(
        ...
        return self._map_messages(
            span_attributes, GEN_AI_INPUT_MESSAGES, SpanAttributes.LLM_INPUT_MESSAGES
        )

    def _map_output_messages(
        self, span_attributes: dict[str, Any]
    ) -> tuple[dict[str, Any], set[str]]:
        """Map output messages to openinference attributes"""
        return self._map_messages(
            span_attributes, GEN_AI_OUTPUT_MESSAGES, SpanAttributes.LLM_OUTPUT_MESSAGES
        )

    def _map_messages(
        self, span_attributes: dict[str, Any], source_key: str, dest_prefix: str
    ) -> tuple[dict[str, Any], set[str]]:
    ...
    def _process_message_parts(
        self, attrs: dict[str, Any], msg_prefix: str, parts: list[dict[str, Any]]
    ) -> None:
        ...
        for part in parts:
            ...
            if part_type == "text":
                ...
                tool_prefix = (
                    f"{msg_prefix}{MessageAttributes.MESSAGE_TOOL_CALLS}.{tool_idx}."
                )
                if tool_id:
                    attrs[f"{tool_prefix}{ToolCallAttributes.TOOL_CALL_ID}"] = str(
                        tool_id
                    )
                if tool_name:
                    attrs[
                ...
                if tool_id:
                    attrs[f"{msg_prefix}{MessageAttributes.MESSAGE_TOOL_CALL_ID}"] = (
                        str(tool_id)
                    )
    ...
    def _map_token_counts(
        ...
        input_tokens = self._get_and_mark(
            span_attributes, GEN_AI_USAGE_INPUT_TOKENS, converted_keys
        )
        if input_tokens is not None:
            attrs[SpanAttributes.LLM_TOKEN_COUNT_PROMPT] = int(input_tokens)

        output_tokens = self._get_and_mark(
            span_attributes, GEN_AI_USAGE_OUTPUT_TOKENS, converted_keys
        )
        if output_tokens is not None:
            attrs[SpanAttributes.LLM_TOKEN_COUNT_COMPLETION] = int(output_tokens)

        if input_tokens is not None and output_tokens is not None:
            attrs[SpanAttributes.LLM_TOKEN_COUNT_TOTAL] = int(input_tokens) + int(
                output_tokens
            )
    ...
    def _map_tool_execution(
        ...
        tool_name = self._get_and_mark(
            span_attributes, GEN_AI_TOOL_NAME, converted_keys
        )
        if tool_name:
            attrs[SpanAttributes.TOOL_NAME] = str(tool_name)

        tool_description = self._get_and_mark(
            span_attributes, GEN_AI_TOOL_DESCRIPTION, converted_keys
        )
        if tool_description:
            attrs[SpanAttributes.TOOL_DESCRIPTION] = str(tool_description)

        tool_call_id = self._get_and_mark(
            span_attributes, GEN_AI_TOOL_CALL_ID, converted_keys
        )
        if tool_call_id:
...
def main():
    ...
    openinference_attributes = conversion_service.convert_otel_span_to_openinference(
        otel_span
    )
