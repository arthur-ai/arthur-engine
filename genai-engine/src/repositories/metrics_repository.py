    ...
    def get_metrics_by_metric_id(self, metric_ids: list[str]) -> list[Metric]:
        ...
        if uncached_ids:
            ...
            database_metrics = self.db_session.query(DatabaseMetric).filter(
                DatabaseMetric.id.in_(uncached_ids)
            ).all()
        ...
        ordered_metrics = [metrics_dict[mid] for mid in metric_ids if mid in metrics_dict]
        
        logger.debug(f"Returned {len(ordered_metrics)} metrics ({len(cached_metrics)} from cache, {len(uncached_metrics)} from DB)")
