...
class TraceQueryRequest(BaseModel):
    ...
    @classmethod
    def validate_relevance_scores(cls, value: Optional[float], info: ValidationInfo) -> Optional[float]:
        """Validate that relevance scores are between 0 and 1 (inclusive)."""
        if value is not None:
            if not (0.0 <= value <= 1.0):
                raise ValueError(f"{info.field_name} value must be between 0 and 1 (inclusive)")
        return value
    ...
    @classmethod
    def validate_trace_duration(cls, value: Optional[float], info: ValidationInfo) -> Optional[float]:
        """Validate that trace duration values are positive."""
        if value is not None:
            if value <= 0:
                raise ValueError(f"{info.field_name} value must be positive (greater than 0)")
        return value
