    ...
    def set_config_type(cls, values):
        ...
        if config_class is not None:
            if config_values is None:
                ...
                if "relevance_threshold" in config_values and "use_llm_judge" in config_values and config_values["use_llm_judge"]:
                    raise HTTPException(
                        status_code=400,
                ...
                if "relevance_threshold" in config_values and "use_llm_judge" not in config_values:
                    config_values["use_llm_judge"] = False
                    
                # If neither is set, default to use_llm_judge=True
                if "relevance_threshold" not in config_values and "use_llm_judge" not in config_values:
                    config_values["use_llm_judge"] = True
