...
def test_get_continuous_eval_variables_and_mappings_success(client: GenaiEngineTestClientBase):
    ...
    try:
        ...
        status_code, continuous_eval_variables_and_mappings = client.get_continuous_eval_variables_and_mappings(
            task_id=task_id,
        ...
        assert status_code == 200
        assert continuous_eval_variables_and_mappings.matching_variables == ["test_variable"]
        assert continuous_eval_variables_and_mappings.transform_variables == ["test_variable"]
        assert continuous_eval_variables_and_mappings.eval_variables == ["test_variable"]
    finally:
        client.delete_llm_eval(task_id, llm_eval.name)
        client.delete_transform(transform.id)
        client.delete_task(agentic_task.id)
...
def test_get_continuous_eval_variables_and_mappings_no_matching_variables(client: GenaiEngineTestClientBase):
    ...
    try:
        ...
        assert continuous_eval_variables_and_mappings.transform_variables == ["test_variable2"]
        assert continuous_eval_variables_and_mappings.eval_variables == ["test_variable"]
    finally:
        client.delete_llm_eval(task_id, llm_eval.name)
        client.delete_transform(transform.id)
...
def test_get_continuous_eval_variables_and_mappings_missing_params(client: GenaiEngineTestClientBase):
    ...
    try:
        ...
        assert status_code == 404
        assert f"transform {fake_transform_id} not found" in response.get("detail", "").lower()
        ...
        assert status_code == 404
        assert f"'fake_llm_eval_name' (version '{llm_eval.version}') not found for task '{task_id}'" in response.get("detail", "").lower()
        ...
        assert status_code == 404
        assert f"'{llm_eval.name}' (version '999999') not found for task '{task_id}'" in response.get("detail", "").lower()
    finally:
        client.delete_llm_eval(task_id, llm_eval.name)
        client.delete_transform(transform.id)
        client.delete_task(agentic_task.id)