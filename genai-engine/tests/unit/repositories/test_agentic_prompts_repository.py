...
@pytest.mark.unit_tests
def test_get_db_prompt_by_datetime_success(agentic_prompt_repo, mock_db_session):
    """Test successfully getting a prompt by datetime"""
    task_id = "test_task_id"
    prompt_name = "test_prompt"

    # Create a datetime and ISO string
    target_datetime = datetime(2024, 1, 15, 10, 30, 45)
    prompt_version = target_datetime.isoformat()

    # Create a mock database prompt with matching created_at
    mock_db_prompt = DatabaseAgenticPrompt(
        task_id=task_id,
        name=prompt_name,
        messages=[{"role": "user", "content": "Test"}],
        model_name="gpt-4",
    ...
    mock_filter.filter.return_value = mock_filter
    mock_filter.order_by.return_value = mock_filter
    mock_filter.first.return_value = mock_db_prompt

    # Create a base query (simulating what would be passed to the method)
    base_query = mock_db_session.query(DatabaseAgenticPrompt).filter(
        DatabaseAgenticPrompt.task_id == task_id,
        DatabaseAgenticPrompt.name == prompt_name,
    )

    result = agentic_prompt_repo._get_db_prompt_by_datetime(base_query, prompt_version)

    assert result is not None
    assert result.name == prompt_name
    assert result.created_at == target_datetime


@pytest.mark.unit_tests
def test_get_db_prompt_by_datetime_not_found(agentic_prompt_repo, mock_db_session):
    """Test getting a prompt by datetime when no matching prompt exists"""
    task_id = "test_task_id"
    prompt_name = "test_prompt"

    # Create a datetime for a prompt that doesn't exist
    target_datetime = datetime(2024, 1, 15, 10, 30, 45)
    prompt_version = target_datetime.isoformat()
...
@pytest.mark.unit_tests
def test_get_db_prompt_by_datetime_invalid_format(agentic_prompt_repo, mock_db_session):
    """Test getting a prompt by datetime with invalid format raises ValueError"""
    task_id = "test_task_id"
    prompt_name = "test_prompt"
    ...
    for invalid_version in invalid_versions:
        with pytest.raises(ValueError) as exc_info:
            agentic_prompt_repo._get_db_prompt_by_datetime(base_query, invalid_version)

        assert f"Invalid prompt_version format '{invalid_version}'" in str(
            exc_info.value,
        )
        assert "Must be 'latest', a version number, or an ISO datetime string" in str(
            exc_info.value,
        )


@pytest.mark.unit_tests
def test_get_db_prompt_by_datetime_within_one_second(
    agentic_prompt_repo,
    mock_db_session,
):
    """Test that _get_db_prompt_by_datetime finds prompts within 1 second window"""
    task_id = "test_task_id"
    prompt_name = "test_prompt"
    ...
    # Create a mock database prompt with created_at within 1 second (0.5 seconds difference)
    from datetime import timedelta

    created_datetime = target_datetime + timedelta(seconds=0.5)
    mock_db_prompt = DatabaseAgenticPrompt(
        task_id=task_id,
        name=prompt_name,
        messages=[{"role": "user", "content": "Test"}],
    ...
    # The method should find the prompt even though it's 0.5 seconds off
    assert result is not None
    assert result.name == prompt_name


@pytest.mark.unit_tests
def test_get_db_prompt_by_datetime_orders_by_desc(agentic_prompt_repo, mock_db_session):
    """Test that _get_db_prompt_by_datetime orders results by created_at descending"""
    task_id = "test_task_id"
    prompt_name = "test_prompt"
    ...
    # Create mock prompts (simulating multiple matches within 1 second)
    mock_db_prompt = DatabaseAgenticPrompt(
        task_id=task_id,
        name=prompt_name,
        messages=[{"role": "user", "content": "Latest version"}],
        model_name="gpt-4",
        model_provider="openai",
        created_at=target_datetime,
        version=2,  # Higher version, should be returned
    )
    ...
    mock_order_by.first.return_value = mock_db_prompt
    ...
    # Verify order_by was called (the mock should have been called)
    assert result is not None
