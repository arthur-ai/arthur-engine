...
@pytest.mark.integration_tests
def test_run_rule_types_check_response_model_with_model_name(
    rule_type: RuleType,
    client: GenaiEngineTestClientBase,
):
    _, task = client.create_task(empty_rules=True)
...
@pytest.mark.integration_tests
def test_toxicity_skipped_with_model_name(
    prompt_str: str,
    expected_rule_results: list[RuleResultEnum],
    client: GenaiEngineTestClientBase,
):
...
@pytest.mark.integration_tests
def test_toxicity_configs_with_model_name(
    threshold: float,
    expected_rule_results: list[RuleResultEnum],
    client: GenaiEngineTestClientBase,
):
...
@pytest.mark.integration_tests
def test_rule_result_skipped_with_model_name(client: GenaiEngineTestClientBase):
    status_code, task = client.create_task(empty_rules=True)
    assert status_code == 200
