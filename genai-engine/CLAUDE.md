...
- **Schema types and enums live in schema modules, not repositories**: API wire-format / request-response schema types and enums (anything that appears in an endpoint's request/response contract, e.g. `TaskSortField`) must be defined in the schema modules under `src/schemas/` and imported by repositories and routes — never declared inside a repository file. The canonical example is `TaskSortField` in [`src/schemas/enums.py`](src/schemas/enums.py), imported by `repositories/tasks_repository.py` and `routers/v2/task_management_routes.py`. This keeps the public API surface in one place and prevents the data-access layer from owning wire-format definitions.
